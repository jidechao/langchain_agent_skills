...
class _SuppressConnectionResetFilter(logging.Filter):
    """Suppress benign ConnectionResetError logs on Windows.

    Windows ProactorEventLoop raises ConnectionResetError (WinError 10054)
    when an SSE client disconnects. These are harmless but noisy.
    """
...
def main() -> None:
    ...
    # Suppress noisy ConnectionResetError on Windows
    if sys.platform == "win32":
        for logger_name in ("asyncio", "uvicorn.error"):
            logging.getLogger(logger_name).addFilter(_SuppressConnectionResetFilter())
